...
## 1. 프로젝트 아키텍처 및 기술 스택
- **Language & Framework**: Swift 6, SwiftUI (macOS 14+), Xcode 15+.
- **Package Manager**: 외부 의존성 없음 (macOS 내장 프레임워크 위주). 단, Sparkle 자동 업데이트 및 키체인 접근(`security` 패키지) 활용.
- **Project Structure**:
...
## 4. 자격증명 관리 및 테스트
- 사용자의 API 키나 개인 정보는 코드 내에 절대 하드코딩해서는 안 되며, macOS Keychain 서비스(`security` 유틸리티)를 통해서만 읽고 써야 합니다.
