...
설치된 앱 빠른 실행, 사칙연산(`5 * (3+2)`), 환율 변환(`100 usd`, `5만원`), 빠른 실행 카드.

### 번역 — `⌘⇧C`

![번역 패널](docs/screenshots/Translator.png)

Apple 온디바이스 번역(`Translation` 프레임워크) 기반 한↔영 번역. API 키도 네트워크도 필요 없고, 입력 언어는 자동 감지해 반대 방향으로 번역합니다. 발음 표기 토글, 우측에 최근 번역 30건 히스토리. 입력·결과 모두 *발음 복사*/*결과 복사* 버튼으로 즉시 클립보드.
...
Jira 자격증명과 숨긴 앱을 한 창에서. 워크스페이스 URL만 넣어주면 Jira Cloud ID는 자동 resolve됩니다. 검색 결과에서 호버 시 나오는 작은 숨기기 버튼으로 가린 앱은 **숨긴 앱** 탭에서 한 번에 되돌리기 가능.

### Jira 대시보드 — `⌘⇧J`

![Jira 대시보드](docs/screenshots/Jira.png)

개인이 어사인된 이슈를 *지난 30일 통계 / 이번 주 / 진행 중 스프린트 / 백로그 / 일별 추이* 관점에서 한 화면에 모아 보여줍니다. status는 Atlassian 표준 카테고리(`new`/`indeterminate`/`done`)로 분류해 한국어/영문 워크스페이스 모두 동작하고, Cloud ID는 워크스페이스 slug에서 자동 resolve됩니다. *(스크린샷의 흐릿한 영역은 회사 식별 정보 블러 처리)*
...
5. `⌘,` 로 Settings 열어 Jira 자격증명 입력 (Jira 대시보드를 쓸 때만)
...
| 번역 | 없음 (Apple 온디바이스 번역 — 첫 사용 시 macOS가 언어 모델 다운로드를 물어봅니다) |
| Jira 대시보드 | Jira Cloud 워크스페이스 + 이메일 + API 토큰 |

---

## 🤖 AI 에이전트용 자동 설치 / 트러블슈팅

Antigravity 등 AI 에이전트가 새 macOS에 Lumen을 자율 설치하기 위한 명령어·검증 단계 + 흔한 문제 해결법은 별도 문서로 분리했습니다 → **[docs/agent-install.md](docs/agent-install.md)**
